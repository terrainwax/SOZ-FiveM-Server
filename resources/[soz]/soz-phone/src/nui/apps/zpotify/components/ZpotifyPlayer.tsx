    ...
    const subscriptions = [
      ...
      wavesurfer.on('ready', (durationTime) => {
        ...
        reader.addEventListener('load', async () => {
          if (!props.artiste || !props.music)
            return
          await db.transaction('rw', db.music, async function () {
            const musicObj = await db.music.where({ artiste: props.artiste, musicName: props.music }).first()
            if (!musicObj)
              await db.music.add(new Music(artiste, music, reader.result as string));
          })
        });
