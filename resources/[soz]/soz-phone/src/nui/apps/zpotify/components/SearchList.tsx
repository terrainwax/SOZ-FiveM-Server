...
export const Searchlist: React.FC<{ musics: SozAlbum[]; input: string }> = ({ musics, input }) => {
    ...
    const artisteSearch = unique.filter(obj => {
        return obj.name.toLowerCase().includes(input.toLowerCase());
    });

    const musicSearch = musics.filter(album => {
        return (
            ...
            album.musics.some(music =>
                music.artiste.some(artiste => artiste.name.toLowerCase().includes(input.toLowerCase()))
            )
        );
    });
        ...
        <div className="h-[89%]">
            <div>
                {artisteSearch.length > 0 && (
                    <span className="text-left text-xl font-bold text-white p-3">Artiste</span>
                )}
                <div className="flex overflow-x-auto w-[400px] flex-row no-scrollbar">
                    {artisteSearch.map((artiste, index) => {
                        return (
                            <Card
                                key={artiste.name + index}
                                className="bg-transparent m-2 min-w-[45%] hover:border hover:border-solid hover:border-[#295122]"
                                nonce={undefined}
                                onResize={undefined}
                            ...
                            >
                                <CardHeader
                                    floated={false}
                                    className="h-24 bg-transparent justify-center flex"
                                    nonce={undefined}
                                    onResize={undefined}
                                    onResizeCapture={undefined}
                                >
            ...
            <div className="h-[62%]">
                {musicSearch.length > 0 && (
                    <span className="text-left text-xl font-bold text-white p-3">Musiques / Albums</span>
                )}
                    ...
                    {musicSearch.map((music, index) => {
                        return (
                            ...
                            >
                                <CardHeader
                                    floated={false}
                                    className="m-0 w-1/5 shrink-0 bg-transparent"
                                    nonce={undefined}
                                    onResize={undefined}
                                    onResizeCapture={undefined}
                                >
                                ...
                                <CardBody
                                    ...
                                    onResize={undefined}
                                    onResizeCapture={undefined}
                                    className="align-middle flex p-0 ml-2 w-[300px]"
                                >
                                    <div className="grow">
                                        <Typography
                                            variant="h4"
